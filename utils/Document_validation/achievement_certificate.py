...
class achievement_certificate(BaseModel):
    document_type: Literal[
        "achievement_certificate", "other_document"  # Specify valid document types
    ]
    name: str
...
""" + formatted_text
        ...
        response = chat(
            ...
            model="llama3.3:70b",
            format=achievement_certificate.model_json_schema(),
            options={'temperature': 0.2},
        )

        # Parse and validate the response
        output_json = achievement_certificate.model_validate_json(response.message.content)
