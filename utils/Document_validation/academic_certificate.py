...
class academic_certificate(BaseModel):
    document_type: Literal[
        "academic_certificate", "other_document"  # Specify valid document types
    ]
    name: str
    
def academice_certificate_extract_event_information(formatted_text: str) -> Optional[dict]:
    """
    ...
    try:
        # Prepare the prompt
        prompt = """
Aim: To analyze the provided document and predict if the text is from an academic certificate or not.

Procedure:
- The goal is to classify whether the text is an Academic Certificate or another type of document.
- If the text is an Academic Certificate, extract the following details:
  - Student Name
- If the document is not an Academic Certificate, return the document type as 'other_document' and leave other fields empty.
...
""" + formatted_text
        ...
        response = chat(
            ...
            model="llama3.3:70b",
            format=academic_certificate.model_json_schema(),
            options={'temperature': 0.2},
        )

        # Parse and validate the response
        output_json = academic_certificate.model_validate_json(response.message.content)
...
if __name__ == "__main__":
    ...
    """
    result = academice_certificate_extract_event_information(sample_text)
    if result:
