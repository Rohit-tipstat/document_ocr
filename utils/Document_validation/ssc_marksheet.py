...
class ssc_marksheet(BaseModel):
    document_type: Literal[
        "ssc_marksheet", "other_document"  # Specify valid document types
    ]
    student_name: str
    # total_marks: int
    # board: str
...
def ssc_marksheet_extract_event_information(formatted_text: str) -> Optional[dict]:
    """
...
Procedure:
- The goal is to classify whether the text is a ssc marksheet or another type of document.
- If the text is a ssc marksheet, extract the following details:
  - Student Name
- If the document is not a ssc marksheet, return the document type as 'other_document' and leave other fields empty.
...
""" + formatted_text
        ...
        response = chat(
            ...
            model="llama3.3:70b",
            format=ssc_marksheet.model_json_schema(),
            options={'temperature': 0.2},
        )

        # Parse and validate the response
        output_json = ssc_marksheet.model_validate_json(response.message.content)
