...
class fee_receipt(BaseModel):
    document_type: Literal[
        "fee_structure", "other_document"  # Specify valid document types
    ]
    student_name: str
    # school_or_college_name: str
    # university_name: Optional[str]
    # course_or_program: str
    # semester: str
    # total_fee_amount: int
    # total_fee_paid: int
    # date: str

def fee_receipt_extract_event_information(formatted_text: str) -> Optional[dict]:
    """
...
Procedure:
- The goal is to classify whether the text is a fee receipt or another type of document.
- If the text is a fee receipt, extract the following details:
  - Student Name
- If the document is not a fee receipt, return the document type as 'other_document' and leave other fields empty.
...
""" + formatted_text
        ...
        response = chat(
            ...
            model="llama3.3:70b",
            format=fee_receipt.model_json_schema(),
            options={'temperature': 0.2},
        )

        # Parse and validate the response
        output_json = fee_receipt.model_validate_json(response.message.content)
