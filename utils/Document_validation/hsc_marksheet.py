...
class hsc_marksheet(BaseModel):
    document_type: Literal[
        "hsc_marksheet", "other_document"  # Specify valid document types
    ]
    student_name: str
    # total_marks: int
    # board: str
...
def hsc_marksheet_extract_event_information(formatted_text: str) -> Optional[dict]:
    """
    ...
    try:
        # Prepare the prompt
        prompt = """
Aim: To analyze the provided document and predict if the text is from hsc marksheet or not.

Procedure:
- The goal is to classify whether the text is a hsc marksheet or another type of document.
- If the text is an achievement certificate, extract the following details:
  - Student Name
- If the document is not a hsc_marksheet, return the document type as 'other_document' and leave other fields empty.
...
""" + formatted_text
        ...
        response = chat(
            ...
            model="llama3.3:70b",
            format=hsc_marksheet.model_json_schema(),
            options={'temperature': 0.2},
        )

        # Parse and validate the response
        output_json = hsc_marksheet.model_validate_json(response.message.content)
