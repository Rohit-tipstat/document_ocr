...
class masters_marksheet(BaseModel):
...
def masters_marksheet_extract_event_information(formatted_text: str) -> Optional[dict]:
    """
...
Procedure:
- The goal is to classify whether the text is a master's marksheet or another type of document.
- If the text is an master's marksheet, extract the following details:
  - Student Name
- If the document is not an master's marksheet, return the document type as 'other_document' and leave other fields empty.
...
""" + formatted_text
        ...
        response = chat(
            ...
            model="llama3.3:70b",
            format=masters_marksheet.model_json_schema(),
            options={'temperature': 0.2},
        )

        # Parse and validate the response
        output_json = masters_marksheet.model_validate_json(response.message.content)
